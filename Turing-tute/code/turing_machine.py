...
""" A Turing machine simulator.
    ...
    Example turing machine implemented with this function::
...
class TuringMachine:
    ...
    A machine is instantiated with transitions, start, accept and reject
    states and a blank symbol. We assume that the input and the tape
    alphabet can be deducted from the transitions.

    :param dict transitions: a mapping from (state, symbol) tuples to
    (state, symbol, direction) tuple. Note that in theory δ is a
    transition *function* (in the sense of mathematical functions), but
    we expect a mapping, not a callable. Directions are either 'L'
    ...
    We don't really care what the input alphabet Σ and the tape
    alphabet Γ are for the purpose of this implementation. For a
    particular run of the machine, the tape alphabet is the union of
    the input, symbols used in transitions and the blank symbol.

    The input on the tape is not part of a Turing machine, so it's not
    required on a Turing  machine instantiation. To execute a
    particular machine use the .run() instance method.
    ...
    def __init__(
        ...
        """Exetute the Turing machine for a particular input.

        :param input_: the input that is written on the tape. It's can
        be a list of strings. Or just a string, in which case each
        letter is treated as a symbol.
        ...
        decide how many steps are executed. Actually, we should not
        even bother with this. On the other side, ^C is not the best
        way for a user to tell us to stop. Instead we give the user
        control to execute us one step at a time. This is what Python
        generators are (partially) for. The yield expression suspends
        us and gives control to the caller until he or she decides to
        resume our execution. Have a look to [1] to get familiar with
        the yield keyord and generators, and hopefully never, ever
        ...
        At each step the generator yields a (action, configuration)
        tuple.

        The action is either 'Accept', 'Reject' or None. 'Accept' and
        `Reject` are self explanatory and signal that the input is
        either accepted or rejected the machine stops in these states.
        None is returned in case the machine needs to continue running.
        ...
        [1] http://www.jeffknupp.com/blog/2013/04/07/improve-your-
    ...
    def run(self, input_):
        ...
        while True:
            ...
            state, symbol, direction = self.transitions.get(
                ...
                # All other transitions are to the reject state.
            )
            ...
            if direction == "R":
                ...
                assert direction in "LR", (
                    "L (left) and R (right) are the "
                    "only correct directions to move."
                )
                logging.warning(
                    "An attempt to move left from the leftmost "
                    "cell! The machine stays put."
                )
    ...
    def debug(self, input_, step_limit=100, colored=False):
        for _, configuration in islice(self.run(input_), step_limit):
            print(
